...
describe 'data_models/index.html.erb' do
  let(:first_data_model)  { double(:data_model, id: 1, name: 'Fraud detection', created_at: Time.now, updated_at: Time.now) }
  let(:second_data_model) { double(:data_model, id: 2, name: 'Beer Daiper Detector', created_at: Time.now, updated_at: Time.now) }
  let(:third_data_model)  { double(:data_model, id: 3, name: 'Food Recommender', created_at: Time.now, updated_at: Time.now) }
